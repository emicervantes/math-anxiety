...
```{r}
m1_1 <- df %>% 
        lm(formula = tma ~ cond + mw1 + mw2 + under_avg + HOT + under_avg + si_sum)
summary(m1_1)
```
...
```{r}
m1_2 <- df %>% 
        lm(formula = tma ~ cond + mw_avg + under_avg + HOT + under_avg + si_sum)
summary(m1_2)
```

Features that have significant effects on math anxiety (p < 0.05): `mw_1_avg` (positive effect: 1.413) and `si_sum` (negative effect: -6.898e-02)
...
`tma ~ cond + mw_avg + si_sum + mw_avg * cond + si_sum * cond`

```{r}
m1_5 <- df %>% 
        lm(formula = tma ~ cond + mw_avg + si_sum + mw_avg * cond + si_sum * cond)
summary(m1_5)
```
